{
	...
	outputs = { self, nixpkgs, flake-utils }:
		...
		let
			...
			# # Create a custom Flutter wrapper that sets up proper Gradle paths
			# flutterWithFixes = pkgs.writeShellScriptBin "flutter" ''
			...
			# 	mkdir -p "$HOME/.gradle"
			#
			# 	# Run flutter with proper environment
		...
		in {
			devShell = pkgs.mkShell {
				buildInputs = [
					# flutterWithFixes    # Use our custom Flutter wrapper
					pkgs.flutter
					pkgs.jdk17
					pkgs.gradle
					...
					pkgs.nmap          # Add nmap since it's used in connectadb
					connectadb
				...
				shellHook = ''
					...
					echo "Run 'installdeps' to set up Android SDK"
				'';
			};
		});
}
